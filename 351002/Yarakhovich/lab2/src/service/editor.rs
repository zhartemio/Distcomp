...
pub async fn create_editor(
    ...
    match editor.validate() {
        Ok(()) => {
            match state.editor_storage.write().await.create(editor).await {
                Ok(editor) => (
                    StatusCode::CREATED,
...
pub async fn list_editors(State(state): State<ServiceState>) -> impl IntoResponse {
    match state.editor_storage.read().await.list().await {
        Ok(editors) => {
...
pub async fn get_editor(
    ...
    match state.editor_storage.read().await.get(id).await {
...
pub async fn delete_editor(
    ...
    match state.editor_storage.write().await.delete(id).await {
