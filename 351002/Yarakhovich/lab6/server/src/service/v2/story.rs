...
pub async fn create_story(
    ...
    match state.story_storage.create(story).await {
        Ok(story) => (
            StatusCode::CREATED,
...
pub async fn list_stories(State(state): State<ServiceState>) -> impl IntoResponse {
    match state.story_storage.list().await {
        Ok(stories) => {
...
pub async fn get_story(
    ...
    match state.story_storage.get(id).await {
...
pub async fn update_story_id(
    ...
    match story.validate() {
        Ok(()) => {
            let res = state.story_storage.update(id, story).await.unwrap();
...
pub async fn delete_story(
    ...
    match state.story_storage.delete(id).await {
