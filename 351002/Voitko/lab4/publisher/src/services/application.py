...
from src.core.note_kafka import KafkaUnavailableError, get_note_kafka_client
...
class NoteService:
    """Заметки хранятся в discussion; основной транспорт - Kafka, fallback - HTTP."""
    ...
    @staticmethod
    def _handle_kafka_response(payload: dict[str, Any]) -> Any:
        status_code = int(payload.get("statusCode", 500))
        if status_code >= 400:
            err = payload.get("error") or {}
            raise AppError(
                status_code,
                int(err.get("errorCode", 0)),
                str(err.get("errorMessage", "Kafka transport error")),
            )
        return payload.get("data")

    @staticmethod
    def _kafka_request(action: str, data: Optional[dict[str, Any]] = None) -> Any:
        try:
            payload = get_note_kafka_client().request(action=action, data=data)
            return NoteService._handle_kafka_response(payload)
        except TimeoutError:
            raise AppError(504, 50401, "Kafka request timeout")
        except KafkaUnavailableError:
            if settings.KAFKA_ENABLED:
                raise AppError(503, 50301, "Kafka unavailable")
            return None
    ...
    def create(db: Session, dto: NoteRequestTo) -> NoteResponseTo:
        ...
        if settings.KAFKA_ENABLED:
            data = NoteService._kafka_request("create", dto.model_dump())
    ...
    def get_all(db: Session) -> List[NoteResponseTo]:
        if settings.KAFKA_ENABLED:
            data = NoteService._kafka_request("get_all", {}) or []
    ...
    def get_by_id(db: Session, id: int):
        if settings.KAFKA_ENABLED:
            data = NoteService._kafka_request("get", {"id": id})
            return NoteResponseTo.model_validate(data)
    ...
    def update(db: Session, id: int, dto: NoteRequestTo):
        if not news_store.get_by_id(db, dto.newsId):
            raise AppError(400, 40004, "News not found")
        if settings.KAFKA_ENABLED:
            data = NoteService._kafka_request("update", {"id": id, **dto.model_dump()})
            return NoteResponseTo.model_validate(data)
    ...
    def delete(db: Session, id: int):
        if settings.KAFKA_ENABLED:
            NoteService._kafka_request("delete", {"id": id})
            return
    ...
    def by_news_id(db: Session, news_id: int) -> List[NoteResponseTo]:
        if not news_store.get_by_id(db, news_id):
            raise AppError(404, 40402, "News not found")
        if settings.KAFKA_ENABLED:
            data = NoteService._kafka_request("by_news", {"newsId": news_id}) or []
            return [NoteResponseTo.model_validate(x) for x in data]
    ...
    def delete_all_for_news(news_id: int) -> None:
        if settings.KAFKA_ENABLED:
            NoteService._kafka_request("delete_by_news", {"newsId": news_id})
            return
