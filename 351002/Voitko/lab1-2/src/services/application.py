...
def _note_to_response(n: Note) -> NoteResponseTo:
    return NoteResponseTo(id=n.id, newsId=n.news_id, content=n.content)
...
class NoteService:
    @staticmethod
    def create(db: Session, dto: NoteRequestTo):
        news = news_store.get_by_id(db, dto.newsId)
        if not news:
            raise AppError(400, 40004, "News not found")

        n = note_store.save(db, Note(news_id=news.id, news=news, content=dto.content))
        return _note_to_response(n)

    @staticmethod
    def get_all(db: Session) -> List[NoteResponseTo]:
        page = note_store.find_all_page(db, PageRequest())
        return [_note_to_response(n) for n in page.content]

    @staticmethod
    def get_by_id(db: Session, id: int):
        n = note_store.get_by_id(db, id)
        if not n:
    ...
    def update(db: Session, id: int, dto: NoteRequestTo):
        ...
        n.news = news
        n.content = dto.content
        note_store.save(db, n)
        return _note_to_response(n)

    @staticmethod
    def delete(db: Session, id: int):
        if not note_store.delete_by_id(db, id):
            raise AppError(404, 40404, "Note not found")
    ...
    def by_news_id(db: Session, news_id: int) -> List[NoteResponseTo]:
        if not news_store.get_by_id(db, news_id):
            raise AppError(404, 40402, "News not found")
        page = note_store.find_by_news_id_page(db, news_id, PageRequest())
        return [_note_to_response(n) for n in page.content]
