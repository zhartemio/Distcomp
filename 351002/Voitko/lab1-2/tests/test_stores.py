...
from src.domain.models.models import Writer, News, Label, Note
...
def test_note_store_by_news(db_session):
    w = writer_store.save(
        db_session,
        Writer(login="w", password="password12", firstname="a", lastname="b"),
    )
    n = news_store.save(
        db_session,
        News(
    ...
    note_store.save(db_session, Note(news_id=n.id, news=n, content="note1"))
    note_store.save(db_session, Note(news_id=n.id, news=n, content="note2"))

    page = note_store.find_by_news_id_page(db_session, n.id, PageRequest())
