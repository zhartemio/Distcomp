...
public class UserServiceImpl implements UserService {
    ...
    public Page<UserResponseTo> getAll(String loginFilter, Pageable pageable) {
        // Реализация фильтрации, если передан loginFilter [cite: 1606]
    ...
    public UserResponseTo create(UserRequestTo dto) {
        ...
        // Проверка уникальности теперь внутри транзакции
    ...
    public UserResponseTo update(Long id, UserRequestTo dto) {
        ...
        // Проверяем смену логина на конфликт с другими юзерами
    ...
    public void delete(Long id) {
        ...
        // 2. Удаляем напрямую по ID (это надежнее для каскадов в Spring Data JPA)
        repository.deleteById(id);

        // ВАЖНО: Убедись, что тут НЕТ проверки if (!user.getTopics().isEmpty())
    }
