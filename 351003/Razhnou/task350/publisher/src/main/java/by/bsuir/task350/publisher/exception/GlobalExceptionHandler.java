package by.bsuir.task350.publisher.exception;

import by.bsuir.task350.publisher.dto.response.ErrorResponse;
...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ErrorResponse> handleDataIntegrityViolation(Exception exception) {
        return ResponseEntity.status(HttpStatus.FORBIDDEN)
                .body(new ErrorResponse(resolveDataIntegrityMessage(exception), 40301));
    }
    ...
    private String resolveDataIntegrityMessage(Exception exception) {
        String message = exception.getMessage();
        if (message != null && message.toLowerCase().contains("login")) {
            return "User login already exists";
        }
        if (message != null && message.toLowerCase().contains("title")) {
            return "Tweet title already exists";
        }
        if (message != null && message.toLowerCase().contains("name")) {
