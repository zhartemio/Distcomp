...
func NewNoteRepository() *NoteRepository {
	...
	s.Query("CREATE TABLE IF NOT EXISTS distcomp.tbl_note (id int PRIMARY KEY, topic_id int, content text, state text)").Exec()
	s.Close()
...
func (r *NoteRepository) Create(n domain.Note) error {
	return r.session.Query("INSERT INTO tbl_note (id, topic_id, content, state) VALUES (?, ?, ?, ?)",
