...
func (k *KafkaConsumer) processPost(ctx context.Context) {
	msg, err := k.reader.FetchPost(ctx)
	if err != nil {
...
func (k *KafkaConsumer) sendResponse(ctx context.Context, replyTo string, response map[string]interface{}) error {
	...
	return k.writer.WritePosts(ctx, kafka.Post{
		Topic: replyTo,
		Value: responseBytes,
	})
}

func (k *KafkaConsumer) commitPost(ctx context.Context, msg kafka.Post) {
	ctx, cancel := context.WithTimeout(ctx, 5*time.Second)
	defer cancel()

	if err := k.reader.CommitPosts(ctx, msg); err != nil {
