...
func (r repo) Create(ctx context.Context, req model.Issue) (model.Issue, error) {
	query := `INSERT INTO tbl_issue (writer_id, title, content) 
	...
	if err != nil {
		...
		if pqErr, ok := err.(*pq.Error); ok && pqErr.Code == "23505" {
			return model.Issue{}, ErrDublicate
		}
...
func (r repo) Get(ctx context.Context, id int64) (model.Issue, error) {
	...
	query := `SELECT id, writer_id, title, content, created, modified FROM tbl_issue WHERE id = $1`
...
func (r repo) Update(ctx context.Context, req model.Issue) (model.Issue, error) {
	...
	query := `UPDATE tbl_issue SET writer_id = $1, title = $2, content = $3, modified = $4 
...
func (r repo) Delete(ctx context.Context, id int64) error {
	r.db.ExecContext(ctx, `DELETE FROM tbl_label WHERE 0 = 0;`)
