...
public class IssueService : BaseService<Issue, IssueRequestTo, IssueResponseTo>, IIssueService
{
    ...
    public override async Task<IssueResponseTo> CreateAsync(IssueRequestTo request)
    {
        ...
        if (!string.IsNullOrEmpty(login) && role == "CUSTOMER" && author.Login != login)
            throw new RestException(403, 29, "Access denied: You can only create issues for your own profile");
    ...
    public override async Task<IssueResponseTo> UpdateAsync(IssueRequestTo request)
    {
        var (login, role) = GetCurrentUser();


        var existingEntity = await _issueRepository.GetByIdAsync(request.Id ?? 0);
        if (existingEntity == null) ThrowNotFound(request.Id ?? 0);

        if (!string.IsNullOrEmpty(login) && role == "CUSTOMER")
        {
            var author = await _authorRepository.GetByIdAsync(existingEntity.AuthorId);
            if (author?.Login != login)
                throw new RestException(403, 28, "Access denied: You are not the owner of this issue");
        }
    ...
    public override async Task<bool> DeleteAsync(long id)
    {
        ...
        if (issue == null) ThrowNotFound(id);


        if (!string.IsNullOrEmpty(login) && role == "CUSTOMER")
        {
            var author = await _authorRepository.GetByIdAsync(issue.AuthorId);
            if (author?.Login != login)
                throw new RestException(403, 29, "Access denied: You are not the owner of this issue");
        }
