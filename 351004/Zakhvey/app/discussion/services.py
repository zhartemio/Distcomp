...
class DiscussionService:
    ...
    def create(dto: CommentRequestTo) -> CommentResponseTo:
        ...
        now = datetime.utcnow()
        # уникальный id на уровне миллисекунд (простая эмитация bigint ID)
    ...
    def delete(comment_id: int):
        res = CommentByIdModel.objects.filter(id=comment_id).first()
        if res:
            # удаляем из обоих пространств хранения
            try:
    ...
    def update(dto: CommentRequestTo) -> CommentResponseTo:
        ...
        now = datetime.utcnow()
        # Обновить в основной таблице
        CommentModel.objects.filter(issue_id=res.issue_id, country=res.country, id=res.id).update(
            content=dto.content,
            modified=now
        )
        # Обновить во второй таблице индексируемой по id
        res.update(content=dto.content, modified=now)
