...
using namespace discussion;
using namespace discussion::dto;
...
void PostController::GetPostsByIssueId(const HttpRequestPtr& req, std::function<void(const HttpResponsePtr&)>&& callback, uint64_t issueId)
{
    HttpResponsePtr httpResponse = HttpResponse::newHttpResponse();
    std::cout << "[INFO] GetPostsByIssueId called for issueId: " << issueId << std::endl;

    try
    {
        std::vector<PostResponseTo> dtos = m_service->GetByIssueId(issueId);
        Json::Value jsonResponse(Json::arrayValue);
        for (auto& dto : dtos)
        {
            jsonResponse.append(dto.toJson());
        ...
        httpResponse->setBody(responseBody);
        httpResponse->setStatusCode(HttpStatusCode::k200OK);
        std::cout << "[INFO] Retrieved " << dtos.size() << " posts for issueId: " << issueId << std::endl;
    }
    catch(const ValidationException& e)
    {
        std::cout << "[ERROR] Validation error: " << e.what() << std::endl;
